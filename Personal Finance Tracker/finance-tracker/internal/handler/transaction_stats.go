...
func (h *Handler) GetTransactionsSummary(c *gin.Context) {
	...
	summary, err := h.transactionService.GetTransactionSummary(user.ID, start, end)
	if err != nil {
...
func (h *Handler) GetTransactionsByCategory(c *gin.Context) {
	...
	categorySummary, err := h.transactionService.GetTransactionsByCategory(user.ID, start, end)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) GetMonthlySummary(c *gin.Context) {
	...
	monthlySummary, err := h.transactionService.GetMonthlySummary(user.ID, year)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
